...
class Excel_attribute:
    ...
    def get_range_value_df(
        self, min_col, min_row, max_col, max_row, excel_ws=None
    ):  # 需要获取除了self.excel_ws之外的其它单元格吗
        """获取指定区域内的单元格值，存储到df,可以行列索引#保护最大行、最大列"""
        cells = []
        if excel_ws == None:
            excel_ws = self.excel_ws
        ...
        add_none_cells = [[None] * (max_col - min_col + 2)] + [
            [None] + row for row in cells
        ]
        return pd.DataFrame(add_none_cells)

    def axising_range_value_df(
        self, range_value_df, ws_row=None, ws_column=None, set_Nan=False
    ):
        """获取/赋空df的特定行/列值"""
        if not set_Nan:
            if ws_row:
                range_value_df.iloc[ws_row, :]
            if ws_column:
                range_value_df.iloc[:, ws_column]
        else:
            if ws_row:
                range_value_df.iloc[ws_row, :] = "__SPECIAL_VALUE__"
            if ws_column:
                range_value_df.iloc[:, ws_column] = "__SPECIAL_VALUE__"
            return range_value_df

    def get_range_cells_dict(
        self, min_col, min_row, max_col, max_row, value_only=True
    ):  # 暂时好像不用了
    ...
    def append_df_to_ws_from_row(
        self, df, start_row, include_index=False, include_header=False
    ):
        """
    ...
    def modify_cell_style(self, cell, style_dict, not_modify_attr=[]):
        ...
        if not_modify_attr:
            possible_attributes = [
                attr for attr in possible_attributes if attr not in not_modify_attr
            ]

        # copy value seperately
        if "value" not in not_modify_attr:
            self.excel_ws.cell(
                row=cell.row, column=cell.column, value=style_dict["value"]
            )
    ...
    def modify_CertainRange_style(self, cell_range, style_dict, not_modify_attr=[]):
        ...
        for row in range(min_row, max_row + 1):
            for col in range(min_col, max_col + 1):
                ...
                self.modify_cell_style(
                    cell, style_dict, not_modify_attr=not_modify_attr
                )

    def modify_MutipleRange_style(
        self, cell_ranges_str, style_dict, not_modify_attr=[]
    ):
        ...
        for cell_range in cell_ranges_list:
            self.modify_CertainRange_style(
                cell_range, style_dict, not_modify_attr=not_modify_attr
            )
...
class SheetCopier:
    ...
    def copy_some_row_attr(
        ...
        if (
            not (
                is_valid_col(min_s_col, max_s_col)
                and is_valid_col(min_t_col, 16384)
                and is_valid_col(max_s_col, 16384)
            )
            or min_s_col > max_s_col
        ):
            raise
        ...
        max_s_col = (
            max(self.source_ws_attr.get_max_row_col()["max_col"])
            if max_s_col == 16384
            else max_s_col
        )
        s_cell_list = [
            f"{get_column_letter(s_col)}{source_row}"
            for s_col in range(min_s_col, max_s_col + 1)
        ]
        t_cell_list = [
            f"{StringPRO.coordinate_from_string(s_cell)[0]}{target_row}"
            for s_cell in s_cell_list
        ]
        ...
        for s_cell, t_cell in s_t_cell_map:
            self.target_ws_attr.modify_cell_style(
                t_cell, source_row_attr[s_cell], not_modify_attr=not_modify_attr
            )
    ...
    def copy_some_col_dropdown(
        self, source_col, target_col, field_row, sep_row=1, dropdown_keyword=""
    ):
    ...
    def copy_all_col_widths(self):
        self.copy_some_col_widths(
            1, max(self.source_ws_attr.get_max_row_col()["max_col"])
        )
...
if "__main__" == __name__:
    ...
    new_wb.save(
        os.path.join(
            StringPRO.get_filepath_variables(colorful_file)["folder_path"],
            "copy_color_sheet.xlsx",
        )
    )
