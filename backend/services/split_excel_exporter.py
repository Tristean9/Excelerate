...
class SplitExcelExporter:
    ...
    def load_excel_parameters(
        ...
        if self.data_start_row > 1:
            for header_row_num in range(1, self.data_start_row + 1):
                self.header_cells_attr.update(
                    self.Xattr.get_row_attributes(header_row_num)
                )
        self.data_row_style.update(
            self.Xattr.get_row_attributes(data_start_row, ["hyperlink"])
        )  # ,"value"
        # print("number_format",self.data_row_style["C3"]["value"])

        data_boundary_dict = self.Xattr.get_max_row_col()
        self.data_max_row, self.data_max_col = min(data_boundary_dict["max_row"]), min(
            data_boundary_dict["max_col"]
        )
        self.all_data_df = self.Xattr.get_range_value_df(
            0, self.data_start_row, self.data_max_col, self.data_max_row
        ).iloc[1:, 1:]
    ...
    def split_worksheet(self) -> Dict[str, int]:
        ...
        self.split_data_df = {
            value: df.loc[df[reference_column_index] == value, original_columns]
        ...
        return {
            value: split_df.shape[0] for value, split_df in self.split_data_df.items()
        }

    def zip_split_files(self, file_name_dict: Dict[str, str]) -> IO[bytes]:
        # 样式
        """
        ...
        for file_name, data_df in self.split_data_df.items():
            clear_row_nums = (
                max(self.Xattr.get_max_row_col()["max_row"]) - self.data_start_row + 1
            )
            ...
            for col in range(1, data_max_col + 1):
                ...
                data_col_range = (
                    f"{col_letter}{self.data_start_row}:{col_letter}{data_max_row}"
                )
            ...
            for col_fistr_cell, col_range in data_col_ranges.items():
                self.Xattr.modify_CertainRange_style(
                    col_range, self.data_row_style[col_fistr_cell], ["value"]
                )

                self.split_data_excel[file_name_dict[file_name]] = (
                    self.Xio.stream_excel_to_frontend(self.excel_wb)
                )
...
if __name__ == "__main__":
    ...
    # 压缩
    zip_buffer = splitExcelExporter.zip_split_files(
        {
            "前沿交叉学科研究院": "1-叉院.xlsx",
            "地球与空间科学学院": "2-地空.xlsx",
            "城市与环境学院": "3-城环.xlsx",
